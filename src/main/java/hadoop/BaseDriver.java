...
public class BaseDriver {
     ...
     */
    public static boolean initJob(JobInitModel[] jobs)
            throws IOException, ClassNotFoundException, InterruptedException {
        //保存作业运行状态
        int[] status = new int[jobs.length];
        int countJob = 0;
        ...
        for (JobInitModel jobInitModel : jobs) {
            ...
            if (jobInitModel.getOutputFormatClass() != null) {
                job.setOutputFormatClass(jobInitModel.getOutputFormatClass());
            } else {
                job.setOutputFormatClass(TextOutputFormat.class);
            }

            //提交作业
            status[countJob++] = job.waitForCompletion(true) ? 0 : 1;
        }
